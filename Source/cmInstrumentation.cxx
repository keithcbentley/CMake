...
//{
//  std::string const uuid = cmExperimental::DataForFeature(cmExperimental::Feature::m_pInstrumentation).Uuid;
//  this->binaryDir = binary_dir;
//  this->timingDirv1 = cmStrCat(this->binaryDir, "/.cmake/instrumentation-", uuid, "/v1");
//  if (cm::optional<std::string> configDir = cmSystemTools::GetCMakeConfigDirectory()) {
//    this->userTimingDirv1 = cmStrCat(configDir.value(), "/instrumentation-", uuid, "/v1");
//  }
//  this->LoadQueries();
//}
//
...
//    this->hasQuery = this->ReadJSONQueries(cmStrCat(this->timingDirv1, "/query")) ||
//      this->ReadJSONQueries(cmStrCat(this->timingDirv1, "/query/generated"));
//  }
//  if (!this->userTimingDirv1.empty() && cmSystemTools::FileExists(cmStrCat(this->userTimingDirv1, "/query"))) {
//    this->hasQuery = this->hasQuery || this->ReadJSONQueries(cmStrCat(this->userTimingDirv1, "/query"));
//  }
//
//  std::string envVal;
//  if (cmSystemTools::GetEnv("CTEST_USE_INSTRUMENTATION", envVal) && !cmIsOff(envVal)) {
...
//        this->AddQuery(cmInstrumentationQuery::Query::DynamicSystemInformation);
...
//  query.ReadJSON(file, this->errorMsg, this->queries, this->hooks, this->callbacks);
//  if (!this->errorMsg.empty()) {
//    cmSystemTools::Error(cmStrCat(
//      "Could not load instrumentation queries from ", cmSystemTools::GetParentDirectory(file), ":\n", this->errorMsg));
//  }
//}
...
//  }
//  this->WriteInstrumentationJson(root, "query/generated", cmStrCat("query-", n, ".json"));
//}
...
//{
//  return (
//    this->HasHook(cmInstrumentationQuery::Hook::PreBuild) || this->HasHook(cmInstrumentationQuery::Hook::PostBuild));
//}

//int cmInstrumentation::CollectTimingData(cmInstrumentationQuery::Hook hook)
//{
//  // Don't run collection if hook is disabled
//  if (hook != cmInstrumentationQuery::Hook::Manual && this->hooks.find(hook) == this->hooks.end()) {
//    return 0;
//  }
//
//  // Touch index file immediately to claim snippets
//  std::string const& directory = cmStrCat(this->timingDirv1, "/data");
//  std::string const& file_name = cmStrCat("index-", ComputeSuffixTime(), ".json");
...
//      cmStrCat(cb, " \"", index_path, "\""), nullptr, nullptr, nullptr, nullptr, cmSystemTools::OUTPUT_PASSTHROUGH);
//  }
//
...
//  }
//  root["dynamicSystemInformation"][cmStrCat(prefix, "HostMemoryUsed")] = (double)info.GetHostMemoryUsed();
//  root["dynamicSystemInformation"][cmStrCat(prefix, "CPULoadAverage")] = info.GetLoadAverage();
//}

//void cmInstrumentation::GetDynamicSystemInformation(
//  double& memory,
//  double& load)
//{
//  cmsys::SystemInformation info;
...
//  infoRoot["processorClockFrequency"] = (double)info.GetProcessorClockFrequency();
//  infoRoot["processorName"] = info.GetExtendedProcessorName();
//  infoRoot["totalPhysicalMemory"] = static_cast<Json::Value::UInt64>(info.GetTotalPhysicalMemory());
//  infoRoot["totalVirtualMemory"] = static_cast<Json::Value::UInt64>(info.GetTotalVirtualMemory());
...
//{
//  uint64_t timeStart = std::chrono::duration_cast<std::chrono::milliseconds>(systemStart.time_since_epoch()).count();
//  uint64_t duration =
//    std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - steadyStart).count();
...
//  std::unique_ptr<Json::StreamWriter> JsonWriter = std::unique_ptr<Json::StreamWriter>(wbuilder.newStreamWriter());
...
//  if (this->HasQuery(cmInstrumentationQuery::Query::DynamicSystemInformation)) {
//    this->InsertDynamicSystemInformation(root, "after");
//  }
//
//  cmsys::SystemInformation info;
//  std::string file_name = cmStrCat(
//    "test-", this->ComputeSuffixHash(cmStrCat(command_str, info.GetProcessId())), this->ComputeSuffixTime(), ".json");
...
//std::string cmInstrumentation::GetCommandStr(std::vector<std::string> const& args)
//{
...
//std::string cmInstrumentation::ComputeSuffixHash(std::string const& command_str)
//{
...
//  ss << cmts.CreateTimestampFromTimeT(ts, "%Y-%m-%dT%H-%M-%S", true) << '-' << std::setfill('0') << std::setw(4) << tms;
//  return ss.str();
//}
 ...
 **/
//void cmInstrumentation::PrepareDataForCDash(
...
//
//  if (!root.isObject()) {
//    error_msg = cmStrCat("Expected index file ", index_path, " to contain an object");
//    cmSystemTools::Error(error_msg);
//    return;
//  }
//
//  if (!root.isMember("snippets")) {
//    error_msg = cmStrCat("Expected index file ", index_path, " to have a key 'snippets'");
//    cmSystemTools::Error(error_msg);
//    return;
//  }
//
...
//      error_msg = cmStrCat("Expected snippet file ", snippet_path, " to contain an object");
//      cmSystemTools::Error(error_msg);
//      continue;
//    }
//    if (!snippet_root.isMember("role")) {
//      error_msg = cmStrCat("Expected snippet file ", snippet_path, " to have a key 'role'");
//      cmSystemTools::Error(error_msg);
//      continue;
//    }
//
...
//      std::string message = "Unexpected snippet type encountered: " + snippet_role;
...
//        dst_dir = cmStrCat(this->cdashDir, "/build/targets/", snippet_root["target"].asString());
