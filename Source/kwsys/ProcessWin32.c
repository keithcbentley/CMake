...
kwsysProcess* kwsysProcess_New(void)
{
  ...
  /* Set initial status.  */
  cp->m_pState = kwsysProcess_State_Starting;
...
void kwsysProcess_Delete(kwsysProcess* cp)
{
  ...
  if (cp->m_pState == kwsysProcess_State_Executing) {
    if (cp->Detached) {
...
{
  return cp ? cp->m_pState : kwsysProcess_State_Error;
}
...
char const* kwsysProcess_GetErrorString(kwsysProcess* cp)
{
  ...
  } else if (cp->m_pState == kwsysProcess_State_Error) {
...
char const* kwsysProcess_GetExceptionString(kwsysProcess* cp)
{
  ...
  } else if (cp->m_pState == kwsysProcess_State_Exception) {
...
int kwsysProcess_GetStateByIndex(kwsysProcess* cp, int idx)
{
  ...
  return cp->ProcessResults[idx].m_pState;
}
...
char const* kwsysProcess_GetExceptionStringByIndex(kwsysProcess* cp, int idx)
{
  ...
  if (cp->ProcessResults[idx].m_pState == kwsysProcess_StateByIndex_Exception) {
...
void kwsysProcess_Execute(kwsysProcess* cp)
{
  ...
  if (!cp || cp->m_pState == kwsysProcess_State_Executing) {
    return;
  }
  ...
  if (cp->NumberOfCommands < 1) {
    ...
    cp->m_pState = kwsysProcess_State_Error;
    return;
  }
  ...
  cp->m_pState = kwsysProcess_State_Executing;
...
void kwsysProcess_Disown(kwsysProcess* cp)
{
  ...
  if (!cp || !cp->Detached || cp->m_pState != kwsysProcess_State_Executing ||
  ...
  cp->m_pState = kwsysProcess_State_Disowned;
}
                             ...
                             double* userTimeout)
{
  ...
  if (!cp || cp->m_pState != kwsysProcess_State_Executing || cp->Killed ||
      cp->TimeoutExpired) {
...
int kwsysProcess_WaitForExit(kwsysProcess* cp, double* userTimeout)
{
  ...
  if (cp->Killed) {
    /* We killed the child.  */
    cp->m_pState = kwsysProcess_State_Killed;
  } else if (cp->TimeoutExpired) {
    /* The timeout expired.  */
    cp->m_pState = kwsysProcess_State_Expired;
  } else {
    ...
    for (i = 0; i < cp->NumberOfCommands; ++i) {
      ...
      if ((cp->ProcessResults[i].ExitCode & 0xF0000000) == 0xC0000000) {
        ...
        cp->ProcessResults[i].m_pState = kwsysProcess_StateByIndex_Exception;
        ...
        cp->ProcessResults[i].m_pState = kwsysProcess_StateByIndex_Exited;
    ...
    cp->m_pState = cp->ProcessResults[cp->NumberOfCommands - 1].m_pState;
  }
...
void kwsysProcess_Interrupt(kwsysProcess* cp)
{
  int i;
  /* Make sure we are executing a process.  */
  if (!cp || cp->m_pState != kwsysProcess_State_Executing || cp->TimeoutExpired ||
      cp->Killed) {
...
int kwsysProcessInitialize(kwsysProcess* cp)
{
  ...
  for (i = 0; i < cp->NumberOfCommands; i++) {
    cp->ProcessResults[i].ExitException = kwsysProcess_Exception_None;
    cp->ProcessResults[i].m_pState = kwsysProcess_StateByIndex_Starting;
